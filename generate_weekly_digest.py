...
Simple script that:
1. Reads all weekly digests from the Supabase weekly_digests table
2. Generates a static RSS XML file
3. Updates the GitHub Pages index.html

This script does NOT generate any content — content is generated by the
Manus scheduled task and written to Supabase beforehand.

Triggered by:
- GitHub Actions workflow_dispatch (from Manus after writing a new digest)
...
def fetch_all_digests():
    ...
    if not resp.ok:
        print(f"ERROR: Failed to fetch digests: {resp.status_code} {resp.text[:200]}")
        sys.exit(1)
    return resp.json()
